			...
			<lxslt:component prefix="my-ext" functions="formatJson,retrievePrizeTable,getType">
				<lxslt:script lang="javascript">
					...
					function formatJson(jsonContext, translations, prizeTable, prizeValues, prizeNamesDesc)
					{
						...
						r.push('</td>');
						r.push('<td class="tablehead" width="25%">');
 						r.push(getTranslationByName("winValues", translations));
						r.push('</td>');
 						r.push('</tr>');

						var isMatch;
						var multiChars = 'TUVWXYZ';
						var multiValues = [2,5,10,20,50,100,200];
						var multiIndex = -1;
						for(var i = 0; i < outcomeNums.length; ++i)
						{
							...
							multiIndex = multiChars.indexOf(outcomeNums[i]);

							if (multiIndex != -1)
							{
 								r.push((multiValues[multiIndex] * getPrizeAsFloat(convertedPrizeValues[getPrizeNameIndex(prizeNames, outcomePrizes[i])])).toFixed(2));
							}
							else if (isMatch)
							{
								r.push(getPrizeAsFloat(convertedPrizeValues[getPrizeNameIndex(prizeNames, outcomePrizes[i])]));
							}
					...
					{
						if ('TUVWXYZ'.indexOf(outcomeNum) != -1)
						{
